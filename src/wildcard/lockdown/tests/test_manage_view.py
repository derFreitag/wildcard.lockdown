...
class TestManageView(unittest.TestCase):
    ...
    def test_view_is_protected(self):
        logout()
        self.assertRaises(
            Unauthorized, self.portal.restrictedTraverse, "@@manage-lockdown"
        )
    ...
    def test_can_get_options(self):
        ...
        self.assertDictEqual(
            self._adjust_dict_to_json(current_options), options_from_view
        )
    ...
    def _render_view(self):
        view = api.content.get_view(
            name="manage-lockdown", context=self.portal, request=self.request
        )
        result = view()
