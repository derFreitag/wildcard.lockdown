...
- **name(required):** Name of condition that'll show up in the management interface
- **path:** An enabling glob expression. This path is always based off the relative
  Plone site, not the Zope root.
- **request_method:** Enabling HTTP request method
- **portal_type:** Published object's portal type
- **host:** Enabling globing globbing expression for the host name
- **logged_in:** require user to be logged in
- **custom:** Custom function to do manual checks against the request object.
...
Here are some examples using [HTTPie](https://httpie.org) from the command line.
Assume the Plone site is at `http://127.0.0.1:8080/Plone`
and there's an `admin` user with password `admin`.

Set options: `enabled=True`, `activated=['Manager user, 'Web API']` and
